...
return $realInput.val();}else{return $input.val();}};var findInput=function($form,attribute){var $input=$form.find(attribute.input);if($input.length&&$input[0].tagName.toLowerCase()==='div'){return $input.find('input');}else{return $input;}};var updateAriaInvalid=function($form,attribute,hasError){if(attribute.updateAriaInvalid){$form.find(attribute.input).attr('aria-invalid',hasError?'true':'false');}}})(window.jQuery);;function LlamarModal(){$valor=$("#frmingresarcedula-cedula").val();$('#Creando').hide();$('#Generando').hide();$('#Comprobando').hide();$('#Enviando').hide();if($valor!=""){$('#modal-espera-envio-correo').modal('show')
